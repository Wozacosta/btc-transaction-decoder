use btc_transaction_decoder::DecodeError;
use clap::{Parser, Subcommand};

#[derive(Parser)]
#[command(name = "Bitcoin CLI")]
#[command(version = "1.0")]
#[command(about = "A command-line interface for Bitcoin transactions", long_about = None)]
struct Cli {
    #[arg(required= true, help =("(string, required) Raw transaction hex"))]
    transaction_hex: String,
}

// #[derive(Subcommand)]
// enum Commands {
//     /// Returns the block hash at the specified height.
//     Getblockhash {
//         #[arg(required = true, help = "(numeric, required) The height index")]
//         height: u64,
//     },
// }

fn main() -> Result<(), DecodeError> {
    let cli = Cli::parse();
    let transaction_hex = cli.transaction_hex;
    // match &cli.command {
    //     Some(Commands::Getblockhash { height }) => {
    //         print!("Block hash at height {}: ", height);
    //         // let block_hash = get_block_hash(*height)?;
    //         // println!("{}", block_hash);
    //     }
    //     None => {
    //         eprintln!("Error: too few params");
    //     }
    // }

    // let transaction_hex = "";

    let raw = hex::decode(transaction_hex)?;
    let tx = btc_transaction_decoder::decode_transaction(&raw)?;

    println!(
        "Transaction: {}",
        serde_json::to_string_pretty(&tx).unwrap()
    );
    Ok(())
}
